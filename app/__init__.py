...
production (PostgreSQL)
  • Tables must exist before the app starts.
  • `flask db upgrade` is a required pre-deployment step.
  • If tables are missing the app logs a CRITICAL error and refuses to start.
...
Seeding (shop items + achievements)
  • Wrapped in OperationalError guards — safe even if tables somehow don't
    exist at seed time (belt-and-suspenders).
"""
import os
import logging
import importlib
...
def create_app(cfg=None):
    ...
    for bp in [auth_bp, entrance_bp, dashboard_bp, tasks_bp, habits_bp,
               expenses_bp, wellness_bp, shop_bp, profile_bp, api_bp,
               share_bp, growth_bp, legal_bp]:
        flask_app.register_blueprint(bp)

    # ── Error handlers ────────────────────────────────────────────────────────
    @flask_app.errorhandler(403)
    def forbidden(e):
        flask_app.logger.warning('403 Forbidden: %s %s',
    ...
    def rate_limited(e):
        flask_app.logger.warning('429 Rate limited: %s from %s',
                                  flask_request.path, flask_request.remote_addr)
    ...
    with flask_app.app_context():
        # Register all model classes with SQLAlchemy metadata so that
        # db.create_all() and Flask-Migrate can discover every table.
...
def _init_db(flask_app: Flask) -> None:
    ...
    development  → auto-create if tables are absent (first-run convenience)
    production   → tables must already exist; log CRITICAL and abort if not
    """
    ...
    if _tables_exist():
        # Normal startup — schema already in place (migrate has been run)
        return

    if env == 'production':
        flask_app.logger.critical(
            'FATAL: database tables are missing in production. '
            'Run `flask db upgrade` before starting the app.'
        )
        raise RuntimeError(
            'Database schema missing in production. '
            'Run `flask db upgrade` and restart.'
        )

    # Development / default: tables are missing on a fresh clone.
    # Create them automatically so `python run.py` works without any
    # migration commands.  Flask-Migrate is still fully available for
...
def _seed_reference_data(flask_app: Flask) -> None:
    ...
    try:
        ...
        flask_app.logger.warning(
            ...
            'This resolves automatically after `flask db upgrade`.'
        )
